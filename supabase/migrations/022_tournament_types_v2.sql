...
-- 1. Fix RLS en tournament_types via función SECURITY DEFINER
-- 2. requires_exact_score: modo 1X2 (L/E/V) sin resultado exacto
-- 3. pts_penalty_wrong_deduct_draw_outcome: deducción configurable
--    para empate de resultado (no exacto) + penal errado (default 0)
-- ============================================================
...
-- ── 2. Actualizar vista pública con las nuevas columnas ───────
CREATE OR REPLACE VIEW public.tournament_types_public
...
-- ── 3. Función SECURITY DEFINER para admin upsert (bypasa RLS) ─
...
BEGIN
  ...
  ELSE
    INSERT INTO public.tournament_types (
      name, description, pts_exact, pts_outcome,
      pts_penalty_correct, pts_penalty_wrong_deduct,
      pts_penalty_wrong_deduct_draw_outcome, requires_exact_score,
      prediction_lock_hours, show_rival_predictions,
      club_fee_percentage, is_active
    ) VALUES (
      p_name, p_description, p_pts_exact, p_pts_outcome,
...
GRANT EXECUTE ON FUNCTION public.admin_upsert_tournament_type TO authenticated;

-- ── 4. calculate_points actualizado ──────────────────────────
--   Agrega:
--   - p_pts_penalty_wrong_deduct_draw_outcome: deducción para
--     empate de resultado (no exacto) + penal errado (default 0)
--   - p_requires_exact_score: si false, exacto = outcome (modo 1X2)
CREATE OR REPLACE FUNCTION public.calculate_points(
  ...
  p_pts_penalty_wrong_deduct_draw_outcome int     DEFAULT 0,
...
BEGIN
  ...
  -- En modo 1X2 (requires_exact_score = false), exacto = outcome
  IF v_exact AND p_requires_exact_score THEN
    v_pts := p_pts_exact;
  ...
  THEN
    ...
    ELSIF v_exact AND p_requires_exact_score THEN
      -- Resultado exacto empate: aplica deducción standard
      v_pts := v_pts - p_pts_penalty_wrong_deduct;
    ELSE
      -- Empate de resultado pero no exacto (ej: predijo 0-0, salió 2-2):
      -- aplica deducción configurable (default 0, para no castigar)
...
-- ── 5. Trigger actualizado: pasa los nuevos parámetros ────────
...
-- ── 6. create_tournament: incluye nuevos campos en rules ──────
