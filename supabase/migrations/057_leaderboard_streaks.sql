-- ============================================================
-- PRODE QUINCHO - Migration 057
-- Leaderboard: columnas is_hot / is_cold para rachas
-- 🔥 encendido: 2+ exactos recientes O últimos 3 todos con puntos
-- ❄️ frío: 0 puntos totales O últimos 3 sin sumar nada
-- ============================================================
...
RETURNS TABLE (
  ...
  WITH recent_preds AS (
    -- Últimas predicciones de cada usuario, ordenadas del más reciente al más antiguo
    SELECT
      p.user_id,
      p.points_earned,
      ROW_NUMBER() OVER (
  ...
  streak_info AS (
    SELECT
      user_id,
      -- 🔥 encendido:
      --   a) 2+ exactos (>=3 pts) entre los últimos 3 partidos, O
      --   b) últimos 3 jugados todos con puntos (ninguno en 0)
      (
        COUNT(CASE WHEN rn <= 3 AND points_earned >= 3 THEN 1 END) >= 2
        OR (
          COUNT(CASE WHEN rn <= 3 THEN 1 END) >= 3
          AND COUNT(CASE WHEN rn <= 3 AND points_earned = 0 THEN 1 END) = 0
        )
      ) AS is_hot,
      -- ❄️ frío:
      --   a) 0 puntos totales (con al menos 1 pronóstico), O
      --   b) últimos 3 partidos todos en 0
      (
        (SUM(points_earned) = 0 AND COUNT(*) > 0)
        OR (
          COUNT(CASE WHEN rn <= 3 THEN 1 END) >= 3
          AND COUNT(CASE WHEN rn <= 3 AND points_earned > 0 THEN 1 END) = 0
        )
  ...
  GROUP BY pr.id, pr.username, pr.full_name, pr.avatar_url, si.is_hot, si.is_cold
