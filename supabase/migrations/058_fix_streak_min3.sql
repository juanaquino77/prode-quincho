...
-- Fix streaks: mínimo 3 partidos jugados para mostrar ícono
-- Evita falsos positivos en los primeros días del torneo
-- ============================================================
...
RETURNS TABLE (
  ...
  streak_info AS (
    SELECT
      user_id,
      COUNT(*) AS played,
      -- 🔥 encendido: SOLO si tiene >= 3 partidos jugados
      --   a) 2+ exactos en los últimos 3, O
      --   b) últimos 3 todos con puntos
      ...
      -- ❄️ frío: SOLO si tiene >= 3 partidos jugados
      --   a) 0 puntos totales O
      --   b) últimos 3 sin sumar nada
      CASE WHEN COUNT(*) >= 3 THEN (
        SUM(points_earned) = 0
        OR COUNT(CASE WHEN rn <= 3 AND points_earned > 0 THEN 1 END) = 0
